...
for _ in range(t):
    ...
    for i in range(0, 101):
        ...
        if incar == m:
            full = True
        new = []
        for passenger in pincar:
            if passenger[1] == i:
                # pincar.remove(passenger)
                incar -= 1
            else:
                new.append(passenger)
        passenger = new
        print(f"\nAt {i}km, {incar} passengers are in car, and they are {pincar}")
        print(f"Profit at {i}km is {profit}")
    if full:
        print(round(profit), "Cab was full")
    else:
        print(round(profit))
